    ...
    const handleSaveFlow = async (chatflowName) => {
        if (reactFlowInstance) {
            const nodes = reactFlowInstance.getNodes().map((node) => {
                ...
                if (Object.prototype.hasOwnProperty.call(nodeData.inputs, CHRONOS_CREDENTIAL_ID)) {
                    nodeData.credential = nodeData.inputs[CHRONOS_CREDENTIAL_ID]
                    nodeData.inputs = omit(nodeData.inputs, [CHRONOS_CREDENTIAL_ID])
                }
                node.data = {
                    ...nodeData,
                    selected: false
