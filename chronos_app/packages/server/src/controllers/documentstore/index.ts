...
const insertIntoVectorStore = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        getRunningExpressApp().metricsProvider?.incrementCounter(CHRONOS_METRIC_COUNTERS.VECTORSTORE_UPSERT, {
...
const saveVectorStoreConfig = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const body = req.body
        const appDataSource = getRunningExpressApp().AppDataSource
        const apiResponse = await documentStoreService.saveVectorStoreConfig(appDataSource, body, true, '')
        return res.json(apiResponse)
    } catch (error) {
        next(error)
    }
