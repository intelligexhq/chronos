import { handleEscapeCharacters, ICommonObject } from 'chronos-components'
    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(componentNodes, 'customFunction')) {
            try {
                ...
                throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Error running custom function: ${error}`)
            }
        } else {
            throw new InternalChronosError(StatusCodes.NOT_FOUND, `Node customFunction not found`)
        }
    } catch (error) {
