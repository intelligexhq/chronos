    ...
    try {
        ...
        if (Object.prototype.hasOwnProperty.call(appServer.nodesPool.componentCredentials, credentialName)) {
            ...
            if (credInstance.icon === undefined) {
                throw new InternalChronosError(StatusCodes.NOT_FOUND, `Credential ${credentialName} icon not found`)
            }
            ...
            if (credInstance.icon.endsWith('.svg') || credInstance.icon.endsWith('.png') || credInstance.icon.endsWith('.jpg')) {
                ...
                throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Credential ${credentialName} icon is missing icon`)
            }
        } else {
            throw new InternalChronosError(StatusCodes.NOT_FOUND, `Credential ${credentialName} not found`)
        }
    } catch (error) {
        throw new InternalChronosError(
            StatusCodes.INTERNAL_SERVER_ERROR,
