...
const logger = createModuleLogger('documentstore')
...
const deleteLoaderFromDocumentStore = async (
    ...
    try {
        ...
        if (found) {
            ...
            throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Unable to locate loader in Document Store ${entity.name}`)
        }
    } catch (error) {
        throw new InternalChronosError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const getDocumentStoreFileChunks = async (
    ...
    try {
        ...
        if (!chunksWithCount) {
            throw new InternalChronosError(StatusCodes.NOT_FOUND, `Chunks with docId: ${docId} not found`)
        }
...
const deleteDocumentStoreFileChunk = async (storeId: string, docId: string, chunkId: string, workspaceId: string) => {
    try {
        ...
        if (!found) {
            throw new InternalChronosError(StatusCodes.NOT_FOUND, `Document store loader ${docId} not found`)
        }
        ...
        if (!tbdChunk) {
            throw new InternalChronosError(StatusCodes.NOT_FOUND, `Document Chunk ${chunkId} not found`)
        }
...
const deleteVectorStoreFromStore = async (storeId: string, workspaceId: string, docId?: string) => {
    try {
        ...
        if (!entity.embeddingConfig) {
            throw new InternalChronosError(StatusCodes.NOT_FOUND, `Embedding for Document store ${storeId} not found`)
        }

        if (!entity.vectorStoreConfig) {
            throw new InternalChronosError(StatusCodes.NOT_FOUND, `Vector Store for Document store ${storeId} not found`)
        }
...
const _splitIntoChunks = async (
    ...
    try {
        ...
        // doc loader configs
        const nodeData = {
            credential: data.credential || data.loaderConfig['CHRONOS_CREDENTIAL_ID'] || undefined,
...
export const insertIntoVectorStore = async ({
    ...
    try {
        ...
        const indexResult = await _insertIntoVectorStoreWorkerThread(appDataSource, componentNodes, telemetry, data, '')
        return indexResult
    } catch (error) {
        throw new InternalChronosError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const queryVectorStore = async (data: ICommonObject) => {
    try {
        ...
        if (!entity.embeddingConfig) {
            throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Embedding for ${data.storeId} is not configured`)
        }

        if (!entity.vectorStoreConfig) {
            throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Vector Store for ${data.storeId} is not configured`)
        }
        ...
        if (!retriever) {
            throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create retriever`)
        }
        const startMillis = Date.now()
        const results = await retriever.invoke(data.query, undefined)
        if (!results) {
            throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to retrieve results`)
        }
...
const _createEmbeddingsObject = async (
    ...
    if (!embeddingObj) {
        throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create EmbeddingObj`)
    }
    return embeddingObj
}
...
const _createRecordManagerObject = async (
    ...
    if (!recordManagerObj) {
        throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create RecordManager obj`)
    }
...
const upsertDocStore = async (
    ...
    if (data.metadata) {
        try {
            ...
            throw new InternalChronosError(StatusCodes.BAD_REQUEST, `Error: Invalid metadata`)
        }
    }
    ...
    if (docId) {
        ...
        if (!loader) {
            throw new InternalChronosError(StatusCodes.NOT_FOUND, `Document loader ${docId} not found`)
        }
    ...
    if (!loaderName || !loaderId || !loaderConfig) {
        throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Loader not configured`)
    }

    if (!vectorStoreName || !vectorStoreConfig) {
        throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Vector store not configured`)
    }

    if (!embeddingName || !embeddingConfig) {
        throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Embedding not configured`)
    }
...
const generateDocStoreToolDesc = async (docStoreId: string, selectedChatModel: ICommonObject): Promise<string> => {
    try {
        ...
        if (!chunks?.length) {
            throw new InternalChronosError(StatusCodes.NOT_FOUND, `DocumentStore ${docStoreId} chunks not found`)
        }
