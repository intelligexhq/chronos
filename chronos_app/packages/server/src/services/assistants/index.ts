...
const createAssistant = async (requestBody: any, orgId: string): Promise<Assistant> => {
    try {
        ...
        if (!requestBody.details) {
            throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Invalid request body`)
        }
        ...
        if (requestBody.type === 'CUSTOM') {
            ...
            appServer.metricsProvider?.incrementCounter(CHRONOS_METRIC_COUNTERS.ASSISTANT_CREATED, {
        ...
        try {
            ...
            if (!credential) {
                throw new InternalChronosError(StatusCodes.NOT_FOUND, `Credential ${requestBody.credential} not found`)
            }
            ...
            if (!openAIApiKey) {
                throw new InternalChronosError(StatusCodes.NOT_FOUND, `OpenAI ApiKey not found`)
            }
            ...
            throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Error creating new assistant - ${getErrorMessage(error)}`)
        }
        const newAssistant = new Assistant()
        Object.assign(newAssistant, requestBody)
        ...
        appServer.metricsProvider?.incrementCounter(CHRONOS_METRIC_COUNTERS.ASSISTANT_CREATED, { status: CHRONOS_COUNTER_STATUS.SUCCESS })
...
const deleteAssistant = async (assistantId: string, isDeleteBoth: any): Promise<DeleteResult> => {
    try {
        ...
        if (!assistant) {
            throw new InternalChronosError(StatusCodes.NOT_FOUND, `Assistant ${assistantId} not found`)
        }
        ...
        try {
            ...
            if (!credential) {
                throw new InternalChronosError(StatusCodes.NOT_FOUND, `Credential ${assistant.credential} not found`)
            }
            ...
            throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Error deleting assistant - ${getErrorMessage(error)}`)
        }
    } catch (error) {
        throw new InternalChronosError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
/**
 * Gets count of assistants - stub for open source (returns 0 since orgs not used)
 * @param type - Assistant type
 * @param _organizationId - Ignored in open source
 * @returns Count of assistants
 */
async function getAssistantsCountByOrganization(type: AssistantType, _organizationId: string): Promise<number> {
    try {
        const appServer = getRunningExpressApp()
        const assistantsCount = await appServer.AppDataSource.getRepository(Assistant).countBy({ type })
...
const updateAssistant = async (assistantId: string, requestBody: any): Promise<Assistant> => {
    try {
        ...
        try {
            ...
            if (!credential) {
                throw new InternalChronosError(StatusCodes.NOT_FOUND, `Credential ${body.credential} not found`)
            }
            ...
            throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Error updating assistant - ${getErrorMessage(error)}`)
        }
    } catch (error) {
        throw new InternalChronosError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const getTools = async (): Promise<any> => {
    try {
        ...
        throw new InternalChronosError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: assistantsService.getTools - ${getErrorMessage(error)}`)
    }
}
