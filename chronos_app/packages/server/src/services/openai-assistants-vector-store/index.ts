...
import { getFileFromUpload, removeSpecificFileFromUpload } from 'chronos-components'
...
const getAssistantVectorStore = async (credentialId: string, vectorStoreId: string) => {
    try {
        ...
        if (!credential) {
            throw new InternalChronosError(StatusCodes.NOT_FOUND, `Credential ${credentialId} not found in the database!`)
        }
