...
export class Prometheus implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        const serviceName: string = process.env.METRICS_SERVICE_NAME || 'Chronos'
        ...
        try {
            const versionGaugeCounter = new promClient.Gauge({
                name: 'chronos_version_info',
                help: 'Chronos version info.',
                labelNames: ['version'],
                registers: [this.register] // Explicitly set the registry
            })
    ...
    public incrementCounter(counter: CHRONOS_METRIC_COUNTERS, payload: any) {
