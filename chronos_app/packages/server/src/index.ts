...
import { SimpleIdentityManager } from './SimpleIdentityManager'
import { SSEStreamer } from './utils/SSEStreamer'
import { validateAPIKey } from './utils/validateKey'
import { SimpleUser } from './Interface.Auth'
...
export class App {
    ...
    identityManager: SimpleIdentityManager
    ...
    async config() {
        ...
        const chronos_file_size_limit = process.env.CHRONOS_FILE_SIZE_LIMIT || '50mb'
        this.app.use(express.json({ limit: chronos_file_size_limit }))
        this.app.use(express.urlencoded({ limit: chronos_file_size_limit, extended: true }))
        ...
        // SSO initialization (no-op for open source)
        ...
        const packagePath = getNodeModulesPackagePath('chronos-ui')
