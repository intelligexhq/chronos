...
function elapsed(run: Run) {
    ...
    const endTime = typeof run.end_time === 'number' ? run.end_time : new Date(run.end_time).getTime()
    const startTime = typeof run.start_time === 'number' ? run.start_time : new Date(run.start_time).getTime()
    const elapsedMs = endTime - startTime
    return `${elapsedMs.toFixed(2)}`
}
