...
import { CHRONOS_CHATID, getBaseClasses, getCredentialData, getCredentialParam } from '../../../src/utils'
...
class Milvus_VectorStores implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        if (isFileUploadEnabled && options.chatId) {
            if (milvusFilter) milvusFilter += ` OR ${CHRONOS_CHATID} == "${options.chatId}" OR NOT EXISTS(${CHRONOS_CHATID})`
            else milvusFilter = `${CHRONOS_CHATID} == "${options.chatId}" OR NOT EXISTS(${CHRONOS_CHATID})`
        }
...
const similaritySearchVectorWithScore = async (query: number[], k: number, vectorStore: Milvus, milvusFilter?: string, filter?: string) => {
    ...
    searchResp.results.forEach((result: Record<string, any>) => {
        const fields = {
            pageContent: '',
        ...
        let normalizedScore = result.score as number
        switch (vectorStore.indexCreateParams.metric_type) {
            case MetricType.L2:
                normalizedScore = 1 / (1 + (result.score as number))
                break
            case MetricType.IP:
            case MetricType.COSINE:
                normalizedScore = ((result.score as number) + 1) / 2
                break
        }
